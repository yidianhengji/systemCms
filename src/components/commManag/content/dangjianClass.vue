...
<script>
  ...
  export default {
    ...
    created() {
      this.$api.postAndJson('/backen/classification/queryAll', {flag: 1, type: 5, communityId: sessionStorage.getItem('communityId')}).then(res => {
    ...
    methods: {
      onClickAdd() {
        this.$router.push({path: '/dangjianClassAdd'})
      },
      click(e) {
        let id = e.target.id;
      ...
      update(uuid) {
        this.$router.push({path: '/dangjianClassAdd', query: { uuid: uuid }})
      },
      del(uuid) {
